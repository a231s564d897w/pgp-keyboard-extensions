...
**PGP Keyboard Extensions** — Morphe-compatible Gboard patch source.

This tree is the **full upstream Gboard-patches project** plus the **PGP** feature set
(Custom Key Row, Unitto floating hub, PGP Pass overlay). Public name is **PGP** so it
is not confused with the original Gboard Patches product.

- Morphe source / `.mpp` builds use this full repository (see `.github/workflows`).
- Internal packages remain `dev.jason.gboardpatches.*` for bytecode ABI compatibility.
- User-facing labels: **PGP**, **PGP Key Row**, **PGP Pass**, **PGP Debug**.

## PGP features (added)

- Multi-row **PGP Key Row** (presets, visual editor, Access Points)
- **Unitto** floating hub (Compose module + cached web fallback)
- **PGP Pass** floating credentials + Autofill service skeleton
- InputConnection bridge + in-app debug panel

See `PGP_STATUS.md`, `PGP_MERGE_GUIDE.md`, `GRADLE_INCLUDE_UNITTO_COMPOSE.md`.

## Upstream base

Built on the public Morphe Gboard patches architecture (original project: Gboard Patches).
This repository is an independent packaging for PGP and is not presented as the upstream product.
