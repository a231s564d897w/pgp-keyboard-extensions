...
public final class GboardProtonPassOverlayHost {
    ...
    private static final String PROTON_PASS_PACKAGE = "proton.android.pass";
...
if (sShowing) {
    ...
    private static WindowManager.LayoutParams createLayoutParams(Context context) {
        return dev.jason.gboardpatches.extension.overlay.OverlayWindowHelper.panelParams(
                context, dp(context, 320), WindowManager.LayoutParams.WRAP_CONTENT,
    ...
    private static void openProtonPass(Context context) {
        try {
            Intent launch = context.getPackageManager()
                    .getLaunchIntentForPackage(PROTON_PASS_PACKAGE);
            if (launch != null) {
        ...
        try {
            Intent market = new Intent(Intent.ACTION_VIEW,
                    Uri.parse("market://details?id=" + PROTON_PASS_PACKAGE));
            market.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            context.startActivity(market);
        } catch (Throwable t) {
            Toast.makeText(context, "Proton Pass not installed", Toast.LENGTH_SHORT).show();
        }
    }
