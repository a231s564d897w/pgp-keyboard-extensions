...
public final class GboardCustomTabsProviderResolver {
    ...
    static List<String> launchCandidates(Context context, String selectedProvider, String url) {
        ...
        // Prefer Iceraven when installed (user's real browser for Custom Tabs).
        final String ICERAVEN = "io.github.forkmaintainers.iceraven";
        try {
            context.getPackageManager().getPackageInfo(ICERAVEN, 0);
            if (!result.contains(ICERAVEN)) {
                result.add(0, ICERAVEN);
            }
        } catch (Throwable ignored) {
        }
