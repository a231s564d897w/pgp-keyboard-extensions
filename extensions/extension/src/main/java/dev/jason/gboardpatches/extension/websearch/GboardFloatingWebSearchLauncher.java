...
public final class GboardFloatingWebSearchLauncher {
    ...
    public static boolean open(Context context,
        ...
        try {
            ...
            // Prefer Iceraven Custom Tabs with the same INITIAL_ACTIVITY_HEIGHT_PX
            // extras Floating Web Search already uses for Chrome.
            try {
                String selected = settings.getBrowserProvider();
                if (selected == null
                        || GboardFloatingWebSearchSettings.BROWSER_SYSTEM_DEFAULT.equals(selected)
                        || selected.contains("iceraven")
                        || selected.contains("firefox")) {
                    String iceraven = "io.github.forkmaintainers.iceraven";
                    try {
                        launchContext.getPackageManager().getPackageInfo(iceraven, 0);
                        OneShotLaunch ice = new OneShotLaunch(launchContext, iceraven, settings);
                        if (ice.bind()) {
                            return true;
                        }
                        // Freeform bottom-sheet with same height if CCT service bind fails
                        Intent view = new Intent(Intent.ACTION_VIEW,
                                android.net.Uri.parse(settings.getStartPageUrl()));
                        view.setPackage(iceraven);
                        FloatingHeightHelper.putCustomTabHeightExtras(view, launchContext);
                        if (FloatingHeightHelper.startFreeformBottomSheet(launchContext, view)) {
                            return true;
                        }
                    } catch (Throwable ignoredIce) {
                    }
                }
