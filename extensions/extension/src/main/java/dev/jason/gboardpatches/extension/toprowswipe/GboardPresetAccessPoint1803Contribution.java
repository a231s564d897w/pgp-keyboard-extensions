...
public final class GboardPresetAccessPoint1803Contribution {
    ...
    public void register(Object controller, Context context) {
        try {
            ...
            // Single Access Point: cycle / apply active PGP Key Row preset.
            // Per-preset Q–P buttons were redundant with the cycle + settings editor.
    ...
    private static void registerOne(Handles active, Object controller, Context context,
        ...
        active.builderIconResourceMethod.invoke(builder,
                    dev.jason.gboardpatches.extension.accesspoint.AccessPointIcons.keyboard(context));
    ...
    private static final class ActivatePresetAction implements Runnable {
        ...
        public void run() {
            ...
            try {
                if (presetId == null) {
                    // Cycle: each stop can be Number-only, Z–M-only, all rows, Q–P, etc.
                    PresetApplyHelper.cycleAndApply(context);
                } else {
                    PresetApplyHelper.applyById(context, presetId);
                }
            } catch (Throwable ignored) {
            }
        }
