...
public final class GboardTopRowSwipeSettingsFeature
    ...
    public GboardPatchesSettingsContract.Screen buildScreen(
        ...
        try {
            ...
            List<GboardPatchesSettingsContract.Row> slotRows =
                    new ArrayList<GboardPatchesSettingsContract.Row>();
            // Visual editor for the active preset (multi-row aware)
            slotRows.add(new GboardPatchesSettingsContract.CommandRow(
                    "Edit PGP Key Row",
                    "Open the visual keyboard editor. Only enabled rows are shown.",
                    subordinateRowsEnabled,
                    new ShowKeyboardStyleEditorAction(host)));
            // Preset manager entry
            KeyRowPreset activePreset = KeyRowPresetStore.readActivePreset(host.getContext());
            String presetSummary = activePreset != null
                    ? "Active: " + activePreset.name
                    : "Manage presets (add / rename / switch)";
            slotRows.add(new GboardPatchesSettingsContract.CommandRow(
                    "Manage PGP Presets",
                    presetSummary,
                    subordinateRowsEnabled,
                    new ShowPresetManagerAction(host)));
            ...
            transferRows.add(new GboardPatchesSettingsContract.CommandRow(
                    "Export presets",
                    "Copy all PGP Key Row presets as JSON",
                    subordinateRowsEnabled,
                    new ExportPresetsAction(host)));
            transferRows.add(new GboardPatchesSettingsContract.CommandRow(
                    "Import presets",
                    "Paste JSON to replace all PGP Key Row presets",
                    subordinateRowsEnabled,
                    new ImportPresetsAction(host)));

            List<GboardPatchesSettingsContract.Row> advancedRows =
                    new ArrayList<GboardPatchesSettingsContract.Row>();
            advancedRows.add(new GboardPatchesSettingsContract.CommandRow(
                    "Open debug panel",
                    "Floating log viewer for PGP Key Row and overlays",
                    true,
                    new OpenDebugPanelAction(host)));
    ...
    /**
     * Opens the keyboard-style visual editor for the active preset.
     * Saves the full preset (all enabled rows) on confirm, and mirrors the
     * Q–P row back into the legacy 10-slot storage for runtime compatibility.
     */
    private static final class ShowKeyboardStyleEditorAction implements Runnable {
        private final GboardPatchesSettingsContract.FeatureHost host;

        ShowKeyboardStyleEditorAction(GboardPatchesSettingsContract.FeatureHost host) {
            this.host = host;
        }
        ...
        public void run() {
            ...
            try {
                ...
                if (active == null) {
                    ...
                    active = KeyRowPreset.fromLegacySlots(
                            GboardTopRowSwipeSettings.readSlots(context));
                    KeyRowPresetStore.savePreset(context, active);
                    KeyRowPresetStore.writeActivePresetId(context, active.id);
                }
                final KeyRowPreset presetToEdit = active;
                GboardPatchesSettingsContract.showManagedDialog(host, onDismiss ->
                        GboardKeyboardStyleSlotEditorDialog.show(
                                activity,
                                presetToEdit,
                                editedPreset -> {
                                    KeyRowPresetStore.savePreset(context, editedPreset);
                                    CustomKeyRowRuntime.invalidate();
                                    // Mirror Q–P row into legacy storage so the
                                    // existing SoftKey runtime keeps working.
                                    List<KeySlot> qRow =
                                            editedPreset.slotsForRow(RowType.Q_TO_P);
                                        ...
                                        GboardTopRowSwipeSettings.SlotText legacy =
                                                i < qRow.size()
                                                        ...
                                                        : new GboardTopRowSwipeSettings.SlotText(
                                                                "", "");
                                        GboardTopRowSwipeSettings.writeSlot(
                                                context, i, legacy);
                                    }
                                    safeRefresh(host);
                                },
                ...
                Log.w(TAG, "Failed to show keyboard-style key row editor", throwable);
            }
        }
    }

    /**
     * Simple preset manager: list presets, switch active, rename, add, delete.
     */
    private static final class ShowPresetManagerAction implements Runnable {
        private final GboardPatchesSettingsContract.FeatureHost host;

        ShowPresetManagerAction(GboardPatchesSettingsContract.FeatureHost host) {
            this.host = host;
        }
        ...
        public void run() {
            ...
            try {
                List<KeyRowPreset> presets = KeyRowPresetStore.readAll(context);
                String activeId = KeyRowPresetStore.readActivePresetId(context);

                CharSequence[] labels = new CharSequence[presets.size()];
                for (int i = 0; i < presets.size(); i++) {
                    KeyRowPreset p = presets.get(i);
                    String mark = p.id.equals(activeId) ? " ✓" : "";
                    labels[i] = p.name + mark;
                }

                new AlertDialog.Builder(activity)
                        .setTitle("Manage PGP Presets")
                        .setItems(labels, (dialog, which) -> {
                            KeyRowPreset chosen = presets.get(which);
                            // Sub-menu: Activate / Rename / Configure rows / Edit keys / Delete
                            ...
                            new AlertDialog.Builder(activity)
                                    ...
                                    .setItems(actions, (d2, action) -> {
                                        switch (action) {
                                            case 0: // activate
                                                KeyRowPresetStore.writeActivePresetId(
                                                        context, chosen.id);
                                                CustomKeyRowRuntime.invalidate();
                                                // Mirror Q–P into legacy storage
                                                List<KeySlot> qRow =
                                                        chosen.slotsForRow(RowType.Q_TO_P);
                                                for (int i = 0; i < GboardTopRowSwipeSettings.SLOT_COUNT; i++) {
                                                    GboardTopRowSwipeSettings.SlotText legacy =
                                                            i < qRow.size()
                                                                    ? qRow.get(i).toLegacy()
                                                ...
                                                break;
                                            case 1: // rename
                                                showRenameDialog(activity, context, chosen);
                                                break;
                                            case 2: // configure rows
                                                showConfigureRowsDialog(activity, context, chosen);
                                                break;
                                            case 3: // edit keys
                                                GboardPatchesSettingsContract.showManagedDialog(
                                                        host, onDismiss ->
                                                                GboardKeyboardStyleSlotEditorDialog.show(
                                                                        activity, chosen,
                                                                        edited -> {
                                                                            KeyRowPresetStore.savePreset(
                                                                                    context, edited);
                                                                            CustomKeyRowRuntime.invalidate();
                                                                            safeRefresh(host);
                                                                        },
                                            ...
                                            case 4: // delete
                                                if (presets.size() <= 1) {
                                                    ...
                                                    KeyRowPresetStore.deletePreset(
                                                            context, chosen.id);
                                                    CustomKeyRowRuntime.invalidate();
                                                    safeRefresh(host);
                                                }
                        ...
                        .setPositiveButton("Add preset", (d, w) -> {
                            KeyRowPreset neu = new KeyRowPreset(
                                    null,
                                    "Preset " + (presets.size() + 1),
                                    java.util.EnumSet.of(RowType.Q_TO_P),
                                    java.util.Collections.emptyList(),
                                    true,
                                    System.currentTimeMillis());
                            // Seed with default emoji slots
                            KeyRowPreset seeded = KeyRowPreset.fromLegacySlots(
                                    GboardTopRowSwipeSettings.defaultSlots());
                            seeded = new KeyRowPreset(
                                    neu.id, neu.name, seeded.enabledRows,
                                    seeded.slots, true, System.currentTimeMillis());
                            KeyRowPresetStore.savePreset(context, seeded);
                            CustomKeyRowRuntime.invalidate();
                            safeRefresh(host);
                        })
                ...
                Log.w(TAG, "Failed to show preset manager", throwable);
            }
        }

        private void showRenameDialog(Activity activity, Context context, KeyRowPreset preset) {
            ...
            new AlertDialog.Builder(activity)
                    ...
                    .setPositiveButton("Save", (d, w) -> {
                        ...
                        if (!name.isEmpty()) {
                            KeyRowPresetStore.savePreset(context, preset.withName(name));
                            CustomKeyRowRuntime.invalidate();
                            safeRefresh(host);
                        }
                    })
        ...
        /**
         * Multi-choice dialog to enable/disable which physical keyboard rows
         * this preset occupies (Number / Q–P / A–L / Z–M).
         */
        private void showConfigureRowsDialog(Activity activity, Context context,
                KeyRowPreset preset) {
            final RowType[] allRows = {
                    RowType.NUMBER, RowType.Q_TO_P, RowType.A_TO_L, RowType.Z_TO_M
            };
            CharSequence[] labels = new CharSequence[allRows.length];
            boolean[] checked = new boolean[allRows.length];
            for (int i = 0; i < allRows.length; i++) {
                labels[i] = allRows[i].displayName;
                checked[i] = preset.enabledRows.contains(allRows[i]);
            }
            new AlertDialog.Builder(activity)
                    .setTitle("Rows for “" + preset.name + "”")
                    .setMultiChoiceItems(labels, checked, (d, which, isChecked) -> {
                        checked[which] = isChecked;
                    })
                    .setPositiveButton("Save", (d, w) -> {
                        java.util.EnumSet<RowType> selected =
                                java.util.EnumSet.noneOf(RowType.class);
                        for (int i = 0; i < allRows.length; i++) {
                            if (checked[i]) {
                                selected.add(allRows[i]);
                            }
                        }
                        if (selected.isEmpty()) {
                            // Always keep at least Q–P so the preset is usable
                            selected.add(RowType.Q_TO_P);
                            Toast.makeText(activity,
                                    "At least one row is required (Q–P kept)",
                                    Toast.LENGTH_SHORT).show();
                        }
                        KeyRowPreset updated = preset.withEnabledRows(selected);
                        KeyRowPresetStore.savePreset(context, updated);
                        CustomKeyRowRuntime.invalidate();
                        safeRefresh(host);
                    })
                    .setNegativeButton("Cancel", null)
    ...
    /** Legacy single-slot form editor (kept for reference / fallback). */
    ...
    private static final class OpenDebugPanelAction implements Runnable {
        private final GboardPatchesSettingsContract.FeatureHost host;

        OpenDebugPanelAction(GboardPatchesSettingsContract.FeatureHost host) {
            this.host = host;
        }
        ...
        public void run() {
            ...
            Context context = host.getContext();
            try {
                if (dev.jason.gboardpatches.extension.debug.GboardDebugPanel.isShowing()) {
                    dev.jason.gboardpatches.extension.debug.GboardDebugPanel.hide(context);
                } else {
                    dev.jason.gboardpatches.extension.debug.GboardDebugPanel.show(context);
                    dev.jason.gboardpatches.extension.debug.GboardDebugPanel.log(
                            "Settings", "Debug panel opened from PGP Key Row settings");
                }
            } catch (Throwable t) {
                Log.w(TAG, "Failed to open debug panel", t);
            }
        }
    }
    ...
    private static final class ExportPresetsAction implements Runnable {
        private final GboardPatchesSettingsContract.FeatureHost host;

        ExportPresetsAction(GboardPatchesSettingsContract.FeatureHost host) {
            this.host = host;
        }
        ...
        public void run() {
            ...
            Context context = host.getContext();
            try {
                String json = KeyRowPresetStore.exportToJson(context);
                GboardPatchesSettingsContract.createTextDocument(host,
                        "gboard-key-row-presets.json",
                        "application/json",
                        json,
                        () -> Toast.makeText(context, "Presets exported",
                                Toast.LENGTH_SHORT).show());
            } catch (Throwable throwable) {
                Log.w(TAG, "Failed to export presets", throwable);
            }
        }
    }

    private static final class ImportPresetsAction implements Runnable {
        private final GboardPatchesSettingsContract.FeatureHost host;

        ImportPresetsAction(GboardPatchesSettingsContract.FeatureHost host) {
            this.host = host;
        }
        ...
        public void run() {
            ...
            try {
                GboardPatchesSettingsContract.openTextDocument(host,
                        ...
                        text -> {
                            int count = KeyRowPresetStore.importFromJson(context, text);
                            if (count > 0) {
                                // Mirror active Q–P into legacy storage
                                KeyRowPreset active =
                                        KeyRowPresetStore.readActivePreset(context);
                                if (active != null) {
                                    List<KeySlot> qRow =
                                            active.slotsForRow(RowType.Q_TO_P);
                                    for (int i = 0;
                                            i < GboardTopRowSwipeSettings.SLOT_COUNT; i++) {
                                        GboardTopRowSwipeSettings.SlotText legacy =
                                                i < qRow.size()
                                                        ? qRow.get(i).toLegacy()
                                                        : new GboardTopRowSwipeSettings.SlotText(
                                ...
                                Toast.makeText(context,
                                        "Import failed – invalid preset JSON",
                                        Toast.LENGTH_SHORT).show();
                            }
                        });
            } catch (Throwable throwable) {
                Log.w(TAG, "Failed to import presets", throwable);
            }
        }
    }
