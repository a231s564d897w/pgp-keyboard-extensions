    ...
    private static final class OpenAction implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                // 1) Real Unitto app — same partial height as Floating Web Search (One UI freeform)
                if (dev.jason.gboardpatches.extension.unitto.UnittoIntegration.openAppIfInstalled(context)) {
                    return;
                }
                // 2) Official Unitto PWA overlay at the same height fraction
                try {
                ...
                } catch (Throwable ignoredPwa) {
                }
                // 3) Built-in engine keypad
