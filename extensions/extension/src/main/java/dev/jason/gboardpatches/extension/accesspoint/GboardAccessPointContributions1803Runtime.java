...
import dev.jason.gboardpatches.extension.calculator.GboardCalculatorAccessPoint1803Contribution;
import dev.jason.gboardpatches.extension.manualincognito.GboardManualIncognitoRuntime;
import dev.jason.gboardpatches.extension.protonpass.GboardProtonPassAccessPoint1803Contribution;
...
public final class GboardAccessPointContributions1803Runtime {
    ...
    public static Object includeOrderCatalog(Context context, Object original) {
        ...
        try {
            result = GboardProtonPassAccessPoint1803Contribution.INSTANCE
                    .extendOrderCatalog(context, result);
        } catch (Throwable failure) {
            logFailure("Proton Pass catalog contribution failed", failure);
        }
        try {
            result = GboardCalculatorAccessPoint1803Contribution.INSTANCE
                    .extendOrderCatalog(context, result);
        } catch (Throwable failure) {
            logFailure("Calculator catalog contribution failed", failure);
        }
        try {
            result = dev.jason.gboardpatches.extension.toprowswipe
                    .GboardPresetAccessPoint1803Contribution.INSTANCE
                    .extendOrderCatalog(context, result);
        } catch (Throwable failure) {
            logFailure("Preset catalog contribution failed", failure);
        }
        return result;
    }
    ...
    public static void afterControllerCreated(Object controller, Context context) {
        ...
        try {
            GboardProtonPassAccessPoint1803Contribution.INSTANCE.register(
                    controller, context);
        } catch (Throwable failure) {
            logFailure("Proton Pass controller contribution failed", failure);
        }
        try {
            GboardCalculatorAccessPoint1803Contribution.INSTANCE.register(
                    controller, context);
        } catch (Throwable failure) {
            logFailure("Calculator controller contribution failed", failure);
        }
        try {
            dev.jason.gboardpatches.extension.toprowswipe
                    .GboardPresetAccessPoint1803Contribution.INSTANCE.register(
                    controller, context);
        } catch (Throwable failure) {
            logFailure("Preset controller contribution failed", failure);
        }
    }
